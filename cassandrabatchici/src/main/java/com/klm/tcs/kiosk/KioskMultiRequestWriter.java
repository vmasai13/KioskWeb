...
public class KioskMultiRequestWriter implements ResourceAwareItemWriterItemStream<RawRecord> {
	...
	public void write(List<? extends RawRecord> items) throws Exception {
		...
		for (RawRecord rawRecord : items) {
			...
			// For BLS Kiosks
			if(rawRecord.getRawLine().contains("EMV") && rawRecord.getRawLine().contains("ResponseCode")) {
				toWrite.add(rawRecord.getRawLine());
			} else if (rawRecord.getRawLine().contains("SellProducts")) {
				// For IBM kiosks
				toWrite.add(rawRecord.getRawLine());
			}
